...
class CreateExpense extends StatefulWidget {
  final int id;
  final String title;
  CreateExpense(this.id, this.title);
  @override
  State<CreateExpense> createState() => _CreateExpense(id, title);
}

class _CreateExpense extends State<CreateExpense> {
  final int id;
  final String title;
  _CreateExpense(this.id, this.title);
                        ...
                        child: Column(children: [
                          ...
                          BlocConsumer<ExpenseBloc, ExpenseState>(
                            ...
                            listener: (context, state) {
                              if (state is CreateCategoryExpenseSuccess) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                        'Expense of ${state.amount} is successfully added on $title'),
                                  ),
                                );
                              }
                            },
                            builder: (context, state) {
                              if (state is CreateCategoryExpenseLoading) {
                                return CircularProgressIndicator();
                              }
                              return ElevatedButton(
                                onPressed: () {
                                  if (_formKey.currentState!.validate()) {
                                    expenseBloc.add(CreateCategoryExpenseEvent(
                                        id,
